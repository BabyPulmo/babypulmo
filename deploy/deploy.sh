...
# .env.production is regenerated from GitHub Secrets on every CI deploy
# (see .github/workflows/deploy.yml). It is NOT a maintained source of truth —
# never edit this file by hand on the VPS; the next push to main will overwrite it.
require_env_file() {
  if [ ! -f ".env.production" ]; then
    echo "✗ .env.production not found. This file is regenerated by GitHub Actions"
    echo "  on every push to main; for first-time bring-up before CI is wired,"
    echo "  copy .env.production.example, fill values, and re-run."
    exit 1
  fi
}
